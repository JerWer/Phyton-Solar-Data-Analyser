...
#                os.remove(item)
        ...
        totaltimedict={}
        for i in range(len(file_path_csv)):
            filename=os.path.split(file_path_csv[i])[-1].split('.')[0]
            if 'time' in filename:
#                print(filename)
...
#                        print(tottime)
                    ...
                    xcentr=[]
                    ycentr=[]
                    for item in range(len(DATA[key][key3][key2][0])):
                        if DATA[key][key3][key2][0][item]>positions[-1]-120 and DATA[key][key3][key2][0][item]<positions[-1]+120:
                            xcentr.append(DATA[key][key3][key2][0][item])
                ...
                numbpoints=len(spectcentroid)
                x=range(0,numbpoints*int(float(tottime.split(' ')[2])),int(float(tottime.split(' ')[2])))
                heightposition[key+key3]=[x,height,positions,spectcentroid]
                plt.plot(x,spectcentroid)
                plt.xlabel('Time (s)')
                plt.ylabel('Spectrum centroid (-)')
                plt.savefig(key+key3+'_spectrumcentroid.png',dpi=300)
                plt.close()
